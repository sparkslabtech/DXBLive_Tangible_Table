...
namespace TouchScript.InputSources
{
    ...
    public sealed class TuioInput : InputSource
    {
        ...
        #region Michon Added Properties 
        public TuioObject currentTUIObject;
        [SerializeField] float ObjectRotation;
        [SerializeField] Vector2 ObjectPosition;
        [SerializeField] int currentIndex;
        [SerializeField] bool objectOnScreen = false;

        public float GetCurrentMarkerRotation() => ObjectRotation;

        public Vector2 GetCurrentMarkerPosition() => ObjectPosition;

        public int GetCurrentMarkerIndex() => currentIndex;
        public int CurrentTotalObjs => objectToInternalId.Count;
        public Dictionary<TuioObject, ObjectPointer> GetObjsList => objectToInternalId;

        public void DebugObjsAvailable()
        {
            string msg = "";
            foreach (var item in objectToInternalId)
            {
                msg += $"Puck {item.Key} {item.Value}";
            }
            Debug.Log($"Available Pucks : {objectToInternalId.Count} \n {msg}");

        }

        public string GetObjsAvailableMessage()
        {
            string msg = "";
            foreach (var item in objectToInternalId)
            {
            ...
            return $"Available Pucks : {objectToInternalId.Count} \n {msg}";

        }


        public bool IsObjectOnScreen => objectOnScreen;

        public Action<TuioObject> OnObjectAddedOnScreen;
        public Action<TuioObject> OnObjectUpdateOnScreen;
        public Action<TuioObject> OnRemovedPuck;
        ...
        private void OnBlobAdded(object sender, TuioBlobEventArgs e)
        {
            ...
            Debug.LogAssertion($"+++OnBlobAdded \n Puck {entity.Id}");
        }
        ...
        private void OnObjectAdded(object sender, TuioObjectEventArgs e)
        {
            ...
            lock (this)
            {
                ...
                currentIndex = entity.ClassId;
                currentTUIObject = e.Object;
                OnObjectAddedOnScreen?.Invoke(currentTUIObject);
                //tuioobje
                //Debug.LogAssertion($"+++Object Added \n Puck {entity.ClassId}");
            }
            objectOnScreen = true;
        }
        ...
        private void OnObjectUpdated(object sender, TuioObjectEventArgs e)
        {
            ...
            lock (this)
            {
                ...
                ObjectRotation = touch.Angle;
                //ObjectRotation = 1;//REMOVE LATER
                ObjectPosition = new Vector2(x, y);
                OnObjectUpdateOnScreen?.Invoke(currentTUIObject);

                //Debug.LogAssertion($"+++ObjectUpdated \nPuck: {entity.ClassId} \n position: x:{x}, y:{y} \n rotation {ObjectRotation}");
            }
        }
